package com.yanbang.base.controller;

import java.util.Collection;

import javax.servlet.http.HttpServletRequest;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.context.annotation.Scope;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.servlet.ModelAndView;

import com.yanbang.base.entity.BaseCargo;
import com.yanbang.base.entity.BaseRegion;
import com.yanbang.base.service.IBaseCargoService;
import com.yanbang.base.service.IBaseRegionService;
import com.yanbang.controller.BaseController;
import com.yanbang.view.JsonView;
import com.yanbang.security.entity.SysOrg;
import com.yanbang.security.service.ISysOrgService;
import com.yanbang.util.AssertUtil;
import com.yanbang.util.URL;
import com.yanbang.util.UtilMethod;

/**
 * 业务控制层<br>
 * 
 * 货物信息管理控制器
 * 
 * @author zhouyibin
 * 
 */
@Controller
@RequestMapping("/baseCargo.do")
@Scope("prototype")
public class BaseCargoController extends BaseController {
	@Autowired
	private IBaseCargoService cargoService;

	/**
	 * 货物信息管理界面
	 * 
	 * @param request
	 * @param response
	 * @return
	 * @throws Exception
	 */
	@RequestMapping(params = "action=index")
	public ModelAndView index(HttpServletRequest request, ModelMap model)
			throws Exception {
		return new ModelAndView(URL.baseURL + "base/cargoIndex");
	}

	/**
	 * 组织机构 树初始化
	 * 
	 * @param request
	 * @return
	 * @throws Exception
	 */
	@RequestMapping(params = "action=query")
	public JsonView query(HttpServletRequest request) throws Exception {
		JsonView view = new JsonView();
		try {
			view.setProperty("resultlist", this.getTreeStr());
			view.setProperty("result", "succ");
		} catch (Exception ex) {
			view.setProperty("result", "error");
		}
		return view;
	}

	/**
	 * 保存
	 * 
	 * @param request
	 * @param org
	 * @return
	 * @throws Exception
	 */

	@RequestMapping(params = "action=saveOrg")
	public JsonView saveOrg(HttpServletRequest request, BaseCargo data)
			throws Exception {
		JsonView view = new JsonView();
		try {
			
			if (AssertUtil.isVal(data.getBas_cargo_class_id())) {
				data.setUpdated_by_user(this.getLoginUserName(request));
				cargoService.updByData(data);
			} else {
				data.setCreated_by_user(this.getLoginUserName(request));
				data.setBas_cargo_class_id(UtilMethod.getUUID());
				cargoService.insByData(data);
			}
			view.setProperty("result", "succ");
			view.setProperty("uuid", data.getBas_cargo_class_id());
		} catch (Exception ex) {
			view.setProperty("result", "error");
		}
		return view;
	}

	/**
	 * 删除
	 * 
	 * @param request
	 * @param uuid
	 * @return
	 * @throws Exception
	 */

	@RequestMapping(params = "action=delOrg")
	public JsonView delOrg(HttpServletRequest request, String uuid)
			throws Exception {
		JsonView view = new JsonView();
		try {
			cargoService.delByPk(uuid);
			view.setProperty("result", "succ");
		} catch (Exception ex) {
			view.setProperty("result", "error");
		}
		return view;
	}

	/**
	 * 组织机构pop
	 * 
	 * @param request
	 * @param response
	 * @return
	 * @throws Exception
	 */
	@RequestMapping(params = "action=indexPop")
	public ModelAndView indexPop(HttpServletRequest request, ModelMap model)
			throws Exception {
		return new ModelAndView("base/regionPop");
	}

	/**
	 * 组织机构查询树 (页面跳转)（该页面为内嵌页面）
	 * 
	 * @param request
	 * @param response
	 * @return
	 * @throws Exception
	 */
	@RequestMapping(params = "action=orgQueryTree")
	public ModelAndView orgQueryTree(HttpServletRequest request,
			ModelMap model, String defaultId) throws Exception {
		model.put("defaultId", AssertUtil.isVal(defaultId) ? defaultId : "");
		return new ModelAndView("base/regionQueryTree");
	}

	/**
	 * 组织机构 树初始化(pop)
	 * 
	 * @param request
	 * @return
	 * @throws Exception
	 */
	@RequestMapping(params = "action=queryPop")
	public JsonView queryPop(HttpServletRequest request) throws Exception {
		JsonView view = new JsonView();
		try {
			view.setProperty("resultlist", this.getTreeStr());
			view.setProperty("result", "succ");
		} catch (Exception ex) {
			view.setProperty("result", "error");
		}
		return view;
	}

	/**
	 * 获取Org的树
	 * 
	 * @return
	 * @throws Exception
	 */
	private String getTreeStr() throws Exception {
		StringBuffer str = new StringBuffer();
		str.append("[{'id':'root','pId':'','tip':'货物信息','cargo_class_name_en':'','name':'货物信息','cargo_name':'','cargo_code':''}");
		// 机械表循环，无父节点，则设置pId为root。
		Collection<BaseCargo> list = cargoService.findAllByFlag();
		for (BaseCargo data : list) {
			str.append(",{'id':'" + data.getBas_cargo_class_id())
					.append("', 'pId':'"
							+ (AssertUtil.isVal(data.getParent_id()) ? data
									.getParent_id() : "root"))
					.append("', 'tip':'" + data.getCargo_class_name())
					.append("', 'cargo_class_name_en':'"
							+ data.getCargo_class_name_en())
					.append("', 'name':'" + data.getCargo_class_code() + "_"
							+ data.getCargo_class_name())
					.append("', 'cargo_name':'" + data.getCargo_class_name())
					.append("', 'cargo_code':'" + data.getCargo_class_code())
					.append("'}");
		}
		str.append("]");
		return str.toString();
	}
}
