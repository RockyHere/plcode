	...
	$.ajax({
		...
		success : function(result) {
			if (result.result == "succ") {
				...
				$("#cust_pro").val($('#tempCustPro').val());//企业类型
	...
	$.ajax({
		...
		success : function(result) {
			if (result.result == "succ") {
				...
				objSelectNow.innerHTML = inner;
				if (obj == "state_code") {// 如果是省，则获取市的数据
					$("#state_code").val($('#tempState').val());
					$('#tempState').val("");
					getProvince($('#state_code').val(), "city_code");

				}
				if (obj == "city_code") {// 如果是县市，则获取县的数据
					$("#city_code").val($('#tempCity').val());
					$('#tempCity').val("");
					getProvince($('#city_code').val(), "region_code");
				}
				if (obj == "region_code") {
					$("#region_code").val($('#tempCounty').val());
					$('#tempCounty').val("");
				}
				return false;
			} else {
				tipAlert('error', '提示', '获取省失败');
			}
		},
		error : function(result) {
			tipAlert('error', '提示', '获取省成功');
		}
	});
}

$(document).ready(function() {
	getProvince("790e94d26d1c462ba9abc7e7bfffc000", "state_code");
	getEnterprisetype();
...
// 编辑职位
function editSaveCust() {
	$.ajax({
		type : "POST",
		data : $('#editForm').serialize(),
		dataType : "json",
		url : "/bscp/baseCust.do?action=editSaveSysJob",
		success : function(result) {
			if (result.result == "succ") {
				// 刷新父页面
...
function initConForm() {// 获取联系人信息列表
	$('#sysJobIframe').attr(
			"src",
			"baseCust.do?action=findMutiConditionCon&curPage=1&pageRows=10&crm_cust_id="
					+ $("#crm_cust_id").val());
}
function initReceForm() {// 获取收货人信息列表
	$('#receIframe').attr(
			"src",
			"baseCust.do?action=findMutiConditionRece&curPage=1&pageRows=10&crm_cust_id="
					+ $("#crm_cust_id").val());
}
...
function initAccForm() {// 获取客户二级账号列表
	$('#accIframe').attr(
			"src",
			"baseCust.do?action=findMutiConditionAcc&curPage=1&pageRows=10&crm_cust_id="
					+ $("#crm_cust_id").val());
}
