...
function btnQuery(pagePerRows) {
	queryForm.action = "/bscp/sysMenu.do?action=findByMutilSysMenu&curPage=1&pageRows="+ pagePerRows;
	queryForm.submit();
}
//新增菜单
function funAdd() {
	art.dialog.open('/bscp/sysMenu.do?action=sysMenuAddIndex', {
		id : 'memdiv',
		width : 400,
		height : 260,
		title : '新增菜单'
...
function funEdit(curPage, pageRows) {
	...
	if (strChoseMenuId == null) {
		art.dialog({
			...
			art.dialog.open('/bscp/sysMenu.do?action=sysMenuEditIndex&menuId=' + strChoseMenuId, {
				id : 'memdiv',
				width : 400,
				height : 260,
				title : '编辑菜单'
...
function funMenuEdit() {
	$.ajax({
		...
		dataType : "json",
		url : "/bscp/sysMenu.do?action=sysMenuEdit",
...
function funMenuSave() {
	$.ajax({
		...
		data : $('#frmMenu').serialize(),
		dataType : "json",
		url : "/bscp/sysMenu.do?action=sysMenuSave",
		success : function(result) {
			if (result.result == "succ") {
				art.dialog({
					content : '新增菜单成功',
