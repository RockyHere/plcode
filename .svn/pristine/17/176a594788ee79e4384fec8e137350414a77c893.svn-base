...
<div >
   <div class="treeDiv" style="float:left;width:30%;height:100%;margin-top:-10px;">
       <ul id="treeOrg" class="ztree"></ul>
   </div>
   <div style="margin-top:-1px;">
   	  <iframe id="sysJobIframe" src=""  style="width:70%;height:100%;" ></iframe> 
   </div>
</div> 
...
var setting = {
	...
	$(document).ready(function(){    
		...
		$.ajax({
			type: "POST",
			url: "/bscp/baseIndustry.do?action=queryPop",				
			dataType: "json",
			cache: true,
	...
	function initOrgForm(){
		$('#sysJobIframe').attr("src","baseIndustry.do?action=findMutiConditionChoice&curPage=1&pageRows=10");
	}
	...
	function editOrgForm(treeNode){
		$('#sysJobIframe').attr("src","baseIndustry.do?action=findMutiConditionChoice&curPage=1&pageRows=10&parentId="+treeNode.id);
