...
<!-- <div id="title"> 
   ...
   <span id="tname">行业信息管理</span>
</div> -->
<div >
   <div class="treeDiv" style="float:left;width:35%;height:100%;margin-top:-10px;">
       <ul id="treeOrg" class="ztree"></ul>
   </div>
   <div style="margin-top:-1px;">
   	  <iframe id="sysJobIframe" src=""  style="width:65%;height:100%;" ></iframe> 
   </div>
</div> 
...
var setting = {
	...
	$(document).ready(function(){    
		...
		$.ajax({
			type: "POST",
			url: "/bscp/baseFinance.do?action=queryPop",				
			dataType: "json",
			cache: true,
			success:function(result){
				if(result.result=="succ"){
					...
					art.dialog({
	    			    content: "<span style='font-size:12px'>金融机构初始化失败</span>",			   
	    			    title:"错误",
	    			    cancelVal: '关闭',
	    			    icon: 'error'
	    			});
			...
			error: function(result){
	     		if(result.status==200){
	     			...
	     			art.dialog({
	    			    content: "<span style='font-size:12px'>行业初始化失败!</span>",			   
	    			    title:"错误信息",
	    			    cancelVal: '关闭',
	    			    icon: 'error'
	    			});
	...
	function initOrgForm(){
		$('#sysJobIframe').attr("src","baseFinance.do?action=findMutiConditionChoice&curPage=1&pageRows=10");
	}
	...
	function editOrgForm(treeNode){
		$('#sysJobIframe').attr("src","baseFinance.do?action=findMutiConditionChoice&curPage=1&pageRows=10&parentid="+treeNode.id);
