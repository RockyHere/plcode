	...
	<form id="editForm" method="post">
		<table  width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr style="height:25px">
				<td class="tabSingleHeader">a合同类型名称</td>
				<td>
					 <select id="contractTypeName" name="contractTypeName" style="width:100%">
                      <option value="">===请选择===</option>                   
                     <c:forEach items="${alist}" var="acontractType" varStatus="status">
                      <option value="${acontractType.uuid}">${acontractType.contractTypeName}</option>
                     </c:forEach>
				</td>
			</tr>
			<tr style="height:25px">
				<td class="tabSingleHeader">f合同类型名称</td>
				<td>
					 <select id="contractTypeName" name="contractTypeName" style="width:100%">
                      <option value="">===请选择===</option>                   
                     <c:forEach items="${flist}" var="fcontractType" varStatus="status">
                      <option value="${fcontractType.uuid}">${fcontractType.contractTypeName}</option>
                     </c:forEach>
				</td>
			</tr>
