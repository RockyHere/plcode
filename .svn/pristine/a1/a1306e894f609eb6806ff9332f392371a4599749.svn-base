	...
	<form id="editForm" method="post">
		<table  width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr style="height:25px">
				<td class="tabSingleHeader">a贸易类型名称</td>
				<td>
					 <select id="taskTypeName" name="taskTypeName" style="width:100%">
                      <option value="">===请选择===</option>                   
                     <c:forEach items="${alist}" var="atask" varStatus="status">
                      <option value="${atask.uuid}">${atask.taskTypeName}</option>
                     </c:forEach>
				</td>
			</tr>
			
			<tr style="height:25px">
				<td class="tabSingleHeader">f贸易类型名称</td>
				<td>
					 <select id="taskTypeName" name="taskTypeName" style="width:100%">
                      <option value="">===请选择===</option>                   
                     <c:forEach items="${flist}" var="ftask" varStatus="status">
                      <option value="${ftask.uuid}">${ftask.taskTypeName}</option>
                     </c:forEach>
				</td>
			</tr>
