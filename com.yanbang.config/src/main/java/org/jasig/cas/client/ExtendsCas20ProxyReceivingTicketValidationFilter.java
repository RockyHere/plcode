 ...
 * cas 客户端配置
 * 
 * 修改了连接的地址,硬编码修改为从properties文件灵活配置
 * 
 * @author Tong Baojun
 * 
 */
public class ExtendsCas20ProxyReceivingTicketValidationFilter extends
		AbstractTicketValidationFilter {
	protected static final Log log = LogFactory
			.getLog(ExtendsCas20ProxyReceivingTicketValidationFilter.class);
	...
	/**
	 * CAS ServerName Address
	 */
	private String ServerNameConfigParameter = "http://localhost:8080";
	/**
	 * cas Modul Name
	 */
	private String contextPathConfigParameter = "/cas";
	...
	protected void initInternal(final FilterConfig filterConfig)
		...
		try {
			this.ServerNameConfigParameter = ResourceProperties
					.getPropValueByKey("cas_client", "cas.host");
			this.contextPathConfigParameter = ResourceProperties
					.getPropValueByKey("cas_client", "cas.contextPath");
			log.info("Loaded cas host address:" + ServerNameConfigParameter
					+ " contextPath:" + contextPathConfigParameter);
		} catch (Exception ex) {
			log.error("Load cas Server host address is error");
		}

		this.setServerName(this.ServerNameConfigParameter);
	...
	protected final TicketValidator getTicketValidator(
		...
		final String casServerUrlPrefix = this.ServerNameConfigParameter
				+ this.contextPathConfigParameter;
