...
public class JpdlModelDrawer {
	...
	public BufferedImage draw(JpdlModel jpdlModel) throws IOException {
		...
		g2.setFont(font);
		Map<String, JpdlNode> nodes = jpdlModel.getNodes();
	...
	private Rectangle getCanvasDimension(JpdlModel jpdlModel) {
		...
		Rectangle rect;
		for (JpdlNode node : jpdlModel.getNodes().values()) {
			...
			for (JpdlTransition transition : node.getTransitions()) {
	 ...
	 * @throws IOException
	 */
	private void drawTransition(Map<String, JpdlNode> nodes, Graphics2D g2)
			throws IOException {
		g2.setStroke(DEFAULT_LINE_STROKE);
		g2.setColor(DEFAULT_LINE_STROKE_COLOR);
		for (JpdlNode node : nodes.values()) {
			for (JpdlTransition transition : node.getTransitions()) {
				String to = transition.getTo();
				JpdlNode toNode = nodes.get(to);
				...
				Point p = JpdlGeometry.getRectangleLineCrossPoint(
				...
				p = JpdlGeometry.getRectangleLineCrossPoint(toNode
	 ...
	 * @throws IOException
	 */
	private void drawNode(Map<String, JpdlNode> nodes, Graphics2D g2, Font font)
			throws IOException {
		for (JpdlNode node : nodes.values()) {
			...
			if (nodeInfos.get(node.getType()) != null) {
				...
				//绘制图片上的文字
				if (node.getType().equals("task")||node.getType().equals("state")) {
					int xLabel = (int) (node.getX() + (node.getWitdth() / 2)-20);
					int yLabel = (int) (node.getY() + (node.getHeight() / 2)+35);
