			 ...
			 <table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					<td width="25%">
					    ...
					    <input type="text" name="jobCode" value="${sysJob.jobCode}"  style="width:150px;"/>
					</td>
					<td width="25%">
					    <span id="label">职位名称:</span>
					    <input type="text" name="jobName" value="${sysJob.jobName}"  style="width:150px;"/>
					</td>
					<td width="50%">
...
<table class="table" cellpadding="0" cellspacing="0" width="100%">
	<tr class="table_top" style="height:25px">
		<td class="tabSingleHeader" style="width:10%;">职位代码</td>
	...
	<c:forEach items="${page.pageResult}" var="item" varStatus="status">
		<tr onmouseover="rowmouseover(this);" ondblclick="retSysJobPop('${item.uuid}','${item.jobCode}','${item.jobName}','${item.orgId}','${item.orgCode}','${item.orgName}')"
		   ...
		   <c:if test="${status.count%2==0}">class="tr2" onmouseout="rowmouseout(this,'tr2')"</c:if>>
		  	<td align="left">${item.jobCode}</td>
		  	<td align="left">${item.jobName}</td>
...
function btnPopQuery(pagePerRows) {
	queryForm.action = "/bscp/sysJob.do?action=sysJobPopQry&curPage=1&pageRows="+ pagePerRows;
	queryForm.submit();
}
//pop双击返回
function retSysJobPop(jobId,jobCode,jobName,orgId,orgCode,orgName){
    //传递返回的值
    var parentDialog = window.parent.art.dialog;
    var retData = {"jobId":jobId,"jobCode":jobCode,"jobName":jobName,"orgId":orgId,"orgCode":orgCode,"orgName":orgName};
