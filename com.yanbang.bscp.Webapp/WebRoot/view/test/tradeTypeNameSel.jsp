	...
	<form id="editForm" method="post">
		<table  width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr style="height:25px">
				...
				<td>
					 <select id="tradeName" name="tradeName" style="width:100%">
                      <option value="">===请选择===</option>                   
                     <c:forEach items="${alist}" var="tradeType" varStatus="status">
                      <option value="${tradeType.uuid}">${tradeType.tradeName}</option>
                     </c:forEach>
				</td>
			</tr>
			<tr style="height:25px">
				...
				<td>
					 <select id="tradeName" name="tradeName" style="width:100%">
                      <option value="">===请选择===</option>                   
                     <c:forEach items="${flist}" var="tradeType" varStatus="status">
                      <option value="${tradeType.uuid}">${tradeType.tradeName}</option>
                     </c:forEach>
				</td>
			</tr>
