...
function btnQuery(pagePerRows) {
	queryForm.action = "/bscp/sysRole.do?action=findByMutilSysRole&curPage=1&pageRows="+ pagePerRows;
	queryForm.submit();
}
//添加角色
function funAdd() {
	art.dialog.open('/bscp/sysRole.do?action=sysRoleAddIndex', {
		id : 'memdiv',
		width : 400,
		height : 200,
		title : '新增角色'
...
function funEdit(curPage, pageRows) {
	...
	if (strChoseRoleId == null) {
		art.dialog({
			...
			art.dialog.open('/bscp/sysRole.do?action=sysRoleEditIndex&roleId=' + strChoseRoleId, {
				id : 'memdiv',
				width : 400,
				height : 200,
				title : '编辑角色'
...
function submitRegistForm() {
	$.ajax({
		...
		dataType : "json",
		url : "/bscp/sysRole.do?action=sysRoleSave",
...
function funRoleEdit() {
	var url = "/bscp/sysRole.do?action=sysRoleEdit";
	$.ajax({
		type : "POST",
		data : $('#frmServRegist').serialize(),
		dataType : "json",
...
function fundel(curPage, pageRows) {
	...
	if (strChoseRoleId == null) {
		...
		$.ajax({
			...
			dataType : "json",
			url : "/bscp/sysRole.do?action=deleteSysRole",
			success : function(result) {
				if (result.result == "succ") {
					// 刷新父页面
					var win = art.dialog.open.origin;
...
function funMenu(curPage, pageRows) {
	...
	if (strChoseRoleId == null) {
		...
		art.dialog.data('pageRows', pageRows);
		art.dialog.data('roleId', strChoseRoleId);
		art.dialog.open('/bscp/sysMenu.do?action=sysMenuTree&roleId=' + strChoseRoleId, {
			id : 'memdiv',
			width : 260,
			height : 440,
			title : '分配角色菜单'
...
function btnGrant(){	
	...
	$.ajax({
		type: "POST",
		url: "/bscp/sysRole.do?action=grantRoleMenus",	
