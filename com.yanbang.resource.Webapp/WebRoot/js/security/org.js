...
$(document).ready(function(){    
	...
	$.ajax({
		type: "POST",
		url: "/bscp/sysOrg.do?action=query",				
		dataType: "json",
		cache: true,
...
//		ok : function() {
			...
			$.ajax({
				type: "POST",
				url: "/bscp/sysOrg.do?action=delOrg&uuid="+treeNode.id,
				dataType: "json",
				success:function(result){
					if(result.result=="succ"){
						var treeObj= $.fn.zTree.getZTreeObj("treeOrg");
...
function editOrg(){
	...
	var data="uuid="+uuid+"&parentId="+parentId+"&orgCode="+$('#orgCode').val()+"&orgName="+$('#orgName').val();
	$.ajax({
		type: "POST",
		data :data,
		url: "/bscp/sysOrg.do?action=saveOrg",
		dataType: "json",
		success:function(result){
			if(result.result=="succ"){
				//--------------------------------------------------
				   ...
				   if($('#uuid').val()=="root" || treeNode==null){
					  var ptreeNode=treeObj.getNodeByParam("id",($('#parentId').val()?$('#parentId').val():"root"));	
					  ptreeNode.finalLevel=false;
				      treeObj.updateNode(ptreeNode);
				      var newNode=treeObj.addNodes(ptreeNode,{isParent:false,id:result.uuid, pId:ptreeNode.id, name:$('#orgCode').val()+"_"+$('#orgName').val()});
				      treeObj.selectNode(newNode[0]); 
				   }else{
					   treeNode.name=$('#orgCode').val()+"_"+$('#orgName').val();
				...
				art.dialog({
    			    content: "<span style='font-size:12px'>保存组织机构错误</span>",			   
    			    title:"错误",
    			    cancelVal: '关闭',
    			    icon: 'error'
    			});
